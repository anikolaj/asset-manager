...
class ExcelUtility:
    def __init__(self, portfolio_analyzer: PortfolioAnalyzer) -> None:
        ...
        self.file_name = f"{self.portfolio_analyzer.portfolio.name}_analysis.xlsx"

        self.monthly_expected_return_row = 0
        self.monthly_expected_return_column = 0

        self.monthly_standard_deviation_row = 0
        self.monthly_standard_deviation_column = 0
    ...
    def write_summary(self) -> None:
        ...
        self.set_cell(
            summary_sheet, summary_sheet["A1"], sheet_title, True, YELLOW_BACKGROUND
        )
    ...
    def write_asset_summary(self, current_row: int, summary_sheet: Worksheet) -> None:
        ...
        for equity in self.portfolio_analyzer.portfolio.equities:
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=3),
                equity.ticker,
                False,
                None,
            )
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=4),
                equity.price,
                False,
                None,
            )
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=5),
                equity.shares,
                False,
                None,
            )
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=6),
                total_value,
                False,
                None,
            )
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=7),
                equity.weight,
                False,
                None,
            )
    ...
    def write_portfolio_summary(
        self, current_row: int, summary_sheet: Worksheet
    ) -> None:
        # section summary
        self.set_cell(
            summary_sheet,
        ...
        self.set_cell(
            summary_sheet,
            summary_sheet.cell(row=current_row, column=2),
            "Expected Return",
            False,
            None,
        )
        ...
        for time_interval in [
            ...
            self.set_cell(
                summary_sheet,
                summary_sheet.cell(row=current_row, column=start_column),
                expected_return,
                False,
                None,
            )
        ...
        for time_interval in [
            ...
            standard_deviation = self.portfolio_analyzer.standard_deviation[
                time_interval
            ]
            self.set_cell(
                summary_sheet,
    ...
    def create_summary_border(self, summary_sheet: Worksheet) -> None:
        ...
        for i in range(1, cell_length):
            summary_sheet.cell(row=i, column=1).border = styles.borders.Border(
        ...
        for i in range(1, num_columns):
            ...
            if cell.border.right.style == "thin":
                cell.border = styles.borders.Border(
                    right=styles.borders.Side(style="thin"),
        ...
        for i in range(3, cell_length):
            summary_sheet.cell(row=i, column=7).border = styles.borders.Border(
                right=styles.borders.Side(style="thin")
            )

        # row 7 and row 10 line for portfolio summary
        portfolio_summary_start_row = (
            len(self.portfolio_analyzer.portfolio.equities) + 5
        )
        for i in range(1, num_columns):
            self.set_cell_border(
                summary_sheet.cell(row=portfolio_summary_start_row - 1, column=i)
            )
    ...
    def write_mvl(self) -> None:
        ...
        self.set_cell(
            mvl_sheet,
            mvl_sheet["A1"],
            "Minimum Variance Line - Monthly",
            True,
            YELLOW_BACKGROUND,
        )

        self.set_cell(mvl_sheet, mvl_sheet["A2"], "Standard Deviation", False, None)
        mvl_sheet["A2"].border = styles.borders.Border(
            bottom=styles.borders.Side(style="thin")
        )

        self.set_cell(
            mvl_sheet, mvl_sheet["B2"], "Efficient Expected Return", False, None
        )
        mvl_sheet["B2"].border = styles.borders.Border(
            bottom=styles.borders.Side(style="thin")
        )

        self.set_cell(
            mvl_sheet, mvl_sheet["C2"], "Inefficient Expected Return", False, None
        )
        mvl_sheet["C2"].border = styles.borders.Border(
            bottom=styles.borders.Side(style="thin")
        )
    ...
    def resize_column(self, worksheet: Worksheet, row: int, column: int) -> None:
        ...
        # set because formating with a specified number format takes the original value not the formatted value
        if worksheet.cell(row, column).number_format == "General":
            if worksheet.column_dimensions[column_str].width < len(
                str(worksheet.cell(row, column).value)
            ):
                worksheet.column_dimensions[column_str].width = len(
                    str(worksheet.cell(row, column).value)
                )

    def write_time_statistics(
        self, worksheet: Worksheet, current_row: int, time_interval: Interval
    ) -> None:
        self.set_cell(
            worksheet,
            worksheet.cell(row=current_row, column=2),
            time_interval.value,
            True,
            None,
        )
    ...
    def write_ticker_statistics(
        self, worksheet: Worksheet, current_row: int, time_interval: Interval
    ) -> None:
        self.set_cell(
            worksheet, worksheet.cell(row=current_row, column=3), "Ticker", True, None
        )
        self.set_cell(
            worksheet,
        ...
        self.set_cell(
            worksheet,
            worksheet.cell(row=current_row, column=5),
            "Standard Deviation",
            True,
            None,
        )
        ...
        for eq in self.portfolio_analyzer.portfolio.equities:
            ...
            self.set_cell(
                worksheet,
                worksheet.cell(row=ticker_row, column=3),
                eq.ticker,
                False,
                None,
            )
            ...
            # expected return
            expected_return = self.portfolio_analyzer.ticker_to_timeseries[eq.ticker][
                time_interval
            ].avg_return
            self.set_cell(
                worksheet,
                worksheet.cell(row=ticker_row, column=4),
                expected_return,
                False,
                None,
                "0.0000000000",
            ...
            # standard deviation
            std_dev = self.portfolio_analyzer.ticker_to_timeseries[eq.ticker][
                time_interval
            ].std_dev
            self.set_cell(
                worksheet,
                worksheet.cell(row=ticker_row, column=5),
                std_dev,
                False,
                None,
                "0.0000000000",
    ...
    def write_correlation_matrix(
        ...
        self.set_cell(
            worksheet,
            worksheet.cell(row=current_row, column=current_column),
            "Correlation Matrix",
            True,
            None,
        )
        ...
        for eq in self.portfolio_analyzer.portfolio.equities:
            ...
            ).border = styles.borders.Border(bottom=styles.borders.Side(style="thin"))
        ...
        for i in range(0, len(self.portfolio_analyzer.portfolio.equities)):
            ...
            self.set_cell(
                worksheet,
                worksheet.cell(row=current_row, column=current_column),
                current_equity.ticker,
                False,
                None,
            )
            worksheet.cell(
                row=current_row, column=current_column
            ).border = styles.borders.Border(right=styles.borders.Side(style="thin"))
            ...
            for j in range(0, len(self.portfolio_analyzer.portfolio.equities)):
                ...
                if j == (len(self.portfolio_analyzer.portfolio.equities) - 1):
                    worksheet.cell(
                        row=current_row, column=current_column + 1
                ...
                if i == (len(self.portfolio_analyzer.portfolio.equities) - 1):
                    worksheet.cell(
                        row=current_row + 1, column=current_column
                    ).border = styles.borders.Border(
    ...
    def write_ticker_mvp(
        ...
        self.set_cell(
            worksheet, worksheet.cell(row=current_row, column=4), "Weight", True, None
        )
        self.set_cell(
            worksheet, worksheet.cell(row=current_row, column=5), "Shares", True, None
        )
        self.set_cell(
            worksheet,
            worksheet.cell(row=current_row, column=6),
            "Value Change",
            True,
            None,
        )
        ...
        for i in range(0, len(self.portfolio_analyzer.portfolio.equities)):
            ...
            # shares
            mvp_shares = (
                mvp_weight * self.portfolio_analyzer.portfolio.value
    ...
    def write_time_mvl(
        ...
        mvp_expected_return = mf.calculate_expected_value(
            self.portfolio_analyzer.M[time_interval],
            self.portfolio_analyzer.mvp[time_interval],
        )
        var = mf.calculate_variance(
            self.portfolio_analyzer.C[time_interval],
            self.portfolio_analyzer.mvp[time_interval],
        )
        if var is None:
            var = 0
        mvp_std_dev = round(math.sqrt(var), 8)

        self.set_mvp_entry(
            worksheet,
            current_row,
            mvp_std_dev,
            np.float64(mvp_expected_return),
            np.float64(mvp_expected_return),
        )
        ...
        for i in range(1, 50, 2):
            ...
            res = mf.solve_quadratic_formula(
                self.portfolio_analyzer.mvl_a[time_interval],
                self.portfolio_analyzer.mvl_b[time_interval],
                self.portfolio_analyzer.mvl_c[time_interval] - (sigma_v**2),
            )
            if res is not None:
                m_v1, m_v2 = res
        ...
        std_devs = chart.Reference(
            worksheet, min_col=1, min_row=3, max_col=1, max_row=28
        )
        efficient_returns = chart.Reference(
            worksheet, min_col=2, min_row=3, max_col=2, max_row=28
        )
        ...
        inefficient_returns = chart.Reference(
            worksheet, min_col=3, min_row=3, max_col=3, max_row=28
        )
        series2 = chart.Series(
            inefficient_returns, std_devs, title="Inefficient Frontier"
        )
        ...
        current_std_dev = chart.Reference(
            self.workbook["Summary"],
            min_col=self.monthly_standard_deviation_column,
            min_row=self.monthly_standard_deviation_row,
            max_col=self.monthly_standard_deviation_column,
            max_row=self.monthly_standard_deviation_row,
        )
        current_expected_return = chart.Reference(
            self.workbook["Summary"],
            min_col=self.monthly_expected_return_column,
            min_row=self.monthly_expected_return_row,
            max_col=self.monthly_expected_return_column,
            max_row=self.monthly_expected_return_row,
        )
        series3 = chart.Series(
            current_expected_return, current_std_dev, title="Current Allocation"
        )
    ...
    def set_mvp_entry(
        ...
        self.set_cell(
            worksheet, worksheet.cell(row=row, column=1), sigma_v, False, None
        )
        ...
        worksheet.cell(row=row, column=3).border = styles.borders.Border(
            right=styles.borders.Side(style="thin")
        )
